...
describe("EscalationPage", () => {
  ...
  function renderPage(params: { id: string }, workflowData?: WorkflowDetail, eventsData?: WorkflowWorkflowEvent[]) {
  ...
  it("renders pause state panel when workflow is paused", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
  ...
  it("shows pause reason from workflow detail", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
    expect(screen.getByText("Needs human approval for tool call")).toBeInTheDocument();
  });

  it("shows cycle count, tokens used, and duration in pause panel", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
  ...
  it("does not render pause panel when workflow is not paused", () => {
    renderPage({ id: "wf-123" }, runningWorkflow, []);
    expect(screen.queryByText("Workflow Paused")).not.toBeInTheDocument();
  });

  // 2. Input editor
  it("renders human input textarea when workflow is paused", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
    expect(
  ...
  it("does not render input editor when workflow is not paused", () => {
    renderPage({ id: "wf-123" }, runningWorkflow, []);
    expect(
  ...
  it("renders submit & resume button when paused", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
  ...
  it("renders cancel button when workflow can be cancelled", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
  ...
  it("does not render cancel button for completed workflow", () => {
    const completedWorkflow: WorkflowDetail = {
      ...runningWorkflow,
      status: "completed",
    };
    renderPage({ id: "wf-123" }, completedWorkflow, []);
    expect(screen.queryByRole("button", { name: /Cancel Workflow/i })).not.toBeInTheDocument();
  });

  it("does not render resume-related controls for running workflow", () => {
    renderPage({ id: "wf-123" }, runningWorkflow, []);
    expect(screen.queryByRole("button", { name: /Submit & Resume/i })).not.toBeInTheDocument();
  });

  // 4. History rendering
  it("renders escalation history timeline", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, sampleEscalationEvents);
  ...
  it("shows human input content in history", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, sampleEscalationEvents);
    expect(screen.getByText("Approved, proceed with caution")).toBeInTheDocument();
  });

  it("filters non-escalation events from history", () => {
    renderPage(
      { id: "wf-123" },
      pausedWorkflow,
  ...
  it("shows empty state when no escalation events", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
  ...
  // 5. Loading state
  it("shows loading skeleton while workflow is loading", () => {
    vi.spyOn(useWorkflowHooks, "useWorkflow").mockReturnValue({
      data: undefined,
      isLoading: true,
      error: null,
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <EscalationPage params={{ id: "wf-123" }} />
      </QueryClientProvider>
    );
  ...
  // 6. Error state
  it("shows error panel when workflow fetch fails", () => {
    vi.spyOn(useWorkflowHooks, "useWorkflow").mockReturnValue({
      data: undefined,
      isLoading: false,
  ...
  it("shows status panel for running workflow", () => {
    renderPage({ id: "wf-123" }, runningWorkflow, []);
    expect(screen.getAllByText("running").length).toBeGreaterThan(0);
    expect(screen.getByText(/No human intervention required/)).toBeInTheDocument();
  });

  it("shows status panel for failed workflow with last pause reason", () => {
    renderPage({ id: "wf-123" }, failedWorkflow, []);
    expect(screen.getAllByText("failed").length).toBeGreaterThan(0);
    expect(screen.getByText(/Previous pause before failure/)).toBeInTheDocument();
  });

  // 9. Navigation
  it("renders workflow nav with escalation tab active", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
  ...
  // 10. Shell wrapper
  it("is wrapped in Shell component", () => {
    renderPage({ id: "wf-123" }, pausedWorkflow, []);
